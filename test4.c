...
void draw_mandelbrot(t_mlx *mlx, int XMAX, int YMAX) {
    int x, y;
    for (x = 0; x < XMAX; ++x) {
        for (y = 0; y < YMAX; ++y) {
            double c_re = (x - XMAX / 2.0) * 4.0 / XMAX / mlx->zoom + mlx->offset_x;
            double c_im = (y - YMAX / 2.0) * 4.0 / YMAX / mlx->zoom + mlx->offset_y;
...
int draw_graph(int XMAX,int YMAX, t_data *img)
{
...
int mouse_hook(int button, int x, int y, t_mlx *mlx)
{
    if (button == 4) // Scroll up (zoom in)
        ...
        mlx->zoom /= 1.1;
    mlx->offset_x += (x - 400) / (200.0 * mlx->zoom);
    mlx->offset_y += (y - 300) / (200.0 * mlx->zoom);
    draw_mandelbrot(mlx, 800, 600);
